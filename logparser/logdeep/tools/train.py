...
from logparser.logdeep.dataset.log import log_dataset
from logparser.logdeep.dataset.sample import (
    session_window,
    sliding_window,
    split_features,
)
from logparser.logdeep.tools.utils import (
...
class Trainer:
    def __init__(self, model, options):
        ...
        if self.sample == "sliding_window":
            ...
            if not os.path.exists(scale_path):
                # os.mknod(scale_path)
                open(file=scale_path, mode="w").close()
    ...
    def save_checkpoint(self, epoch, save_optimizer=True, suffix=""):
        ...
        print(f"Save model checkpoint at {save_path}")
    ...
    def train(self, epoch):
        ...
        print(
            "\nStarting epoch: %d | phase: train | ⏰: %s | Learning rate: %f" % (epoch, start, lr)
        )
